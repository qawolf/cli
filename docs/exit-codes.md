...
| `2`  | `invalidArgs` | Commander parse error, unknown subcommand, bad flag value, no runner available, a flow needing a different runner image, a runner whose screen has never started because nothing has run on it, or a runner asked for something it can never do (no screen to see or drive).                                                                               |
| `3`  | `auth`        | Missing or invalid `QAWOLF_API_KEY`.                                                                                                                                                                                                                                                                                                                       |
| `4`  | `network`     | Apex unreachable, GCS download failure, registry unreachable, or a runner that could not serve the request now (unreachable, or a screen that is up but busy or restarting). Usually transient and worth retrying, except `qawolf runner exec`, which also reports a permanent no-live-page condition as `4` — read the message on stderr before retrying. |
