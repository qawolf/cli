...
describe("getAuthConfig", () => {
  ...
  it("reads a blank client id as offering none", async () => {
    const result = await getAuthConfig({
      baseUrl,
      fetch: createFetchMock(jsonResponse({ workOsClientId: "   " })),
    });
  ...
  it.each([
    ...
    // A gateway or a policy answering for the deployment, not the deployment
    // saying it offers no browser sign-in.
    ["an authentication wall", 401],
    ["a forbidden answer", 403],
