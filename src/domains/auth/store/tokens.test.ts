...
describe("saveTokens", () => {
  ...
  it("clears a keychain entry it could not overwrite", async () => {
    ...
    await saveTokens("/config", tokens, memFs);

    // Otherwise the stale entry keeps winning over the file on every load.
  ...
  it("keeps two saves in one process apart", async () => {
    ...
    await Promise.all([
      saveTokens("/config", tokens, memFs),
      saveTokens("/config", other, memFs),
    ]);

    const stored = await memFs.readFile("/config/tokens.json");
    const candidates = [tokens, other].map((t) =>
      JSON.stringify(t, undefined, 2),
    );
    expect(candidates).toContain(stored);
    expect(await memFs.readdir("/config")).toEqual(["tokens.json"]);
  });
  ...
  it("removes the staging file when it cannot be published", async () => {
    ...
    const refusing: Fs = {
      ...memFs,
      rename: async () => {
        throw Object.assign(Error("EACCES: permission denied"), {
          code: "EACCES",
        });
      },
    };
    ...
    let caught: unknown;
    try {
      await saveTokens("/config", tokens, refusing);
    } catch (err) {
      caught = err;
    }

    expect((caught as Error | undefined)?.message).toContain("EACCES");
    expect(await memFs.readdir("/config")).toEqual([]);
  });
  ...
  it("removes a staging file whose write did not complete", async () => {
    ...
    const truncating: Fs = {
      ...
      writeFile: async (path, data) => {
        // Half the record lands, then the disk gives out.
        await memFs.writeFile(path, String(data).slice(0, 20));
        throw Object.assign(Error("ENOSPC: no space left"), { code: "ENOSPC" });
      },
    };

    let caught: unknown;
    try {
      await saveTokens("/config", tokens, truncating);
    } catch (err) {
      caught = err;
    }

    expect((caught as Error | undefined)?.message).toContain("ENOSPC");
    expect(await memFs.readdir("/config")).toEqual([]);
  });
  ...
  it("leaves no partial file behind, and no temporary one", async () => {
    ...
    // The record was never written to its final name; it arrived by rename.
    expect(written).not.toContain("/config/tokens.json");
    expect(await memFs.readdir("/config")).toEqual(["tokens.json"]);
  });
