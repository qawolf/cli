...
export async function resolveOauthToken(
  ...
  if (!refreshed.ok) {
    ...
    // Only this session's replacement is adopted: the same account, client and
    // organization, and a pair that has not itself lapsed. The store is
    // shared, so what landed there may be another person's sign-in.
    const current = await deps.loadTokens(configDir);
    if (
      current.found &&
      current.tokens.refreshToken !== tokens.refreshToken &&
      current.tokens.email === tokens.email &&
      current.tokens.organizationId === tokens.organizationId &&
      current.tokens.clientId === tokens.clientId &&
      current.tokens.expiresAt !== undefined &&
      current.tokens.expiresAt > deps.now()
    ) {
      return {
