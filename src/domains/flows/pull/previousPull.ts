...
// A failed listing fetch must not erase cached IDs or break offline tag queries.
export async function carriedFromPreviousPull(
  envDir: string,
  fs: Fs,
): Promise<{
  tags: FetchedTags | undefined;
  flowIds: Map<string, string> | undefined;
}> {
  const previous = await readManifest(envDir, fs);
  if (typeof previous === "string") {
    return { tags: undefined, flowIds: undefined };
  }
  ...
  for (const flow of previous.flows) {
    ...
    if (flow.flowId !== undefined) flowIds.set(path, flow.flowId);
  }
  return {
    tags:
