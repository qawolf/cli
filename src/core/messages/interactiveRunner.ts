...
export const interactiveRunnerMessages = {
  ...
  launchFailed: (id: string, error: string) =>
    `Launching runner ${id} failed. ${error}. If the request did reach QA Wolf then the runner may be running under that id, so relaunch the same id with qawolf runner launch --id ${id} to attach to it rather than start a second one, or stop it with qawolf runner stop --runner ${id}.`,
