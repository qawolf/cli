  ...
  workos: {
    ...
    metadata: {
      invalidIssuer: (issuer: string) =>
        `The deployment names a sign-in provider that is not a URL: ${issuer}`,
      ...
      missingEndpoint: (field: string) =>
        `The sign-in provider's metadata advertises no ${field}`,
      foreignEndpoint: (field: string) =>
        `The sign-in provider's metadata puts ${field} on a different origin from the issuer`,
    },
  },
  bundle: {
    linkExpired:
