...
That screen plays only for the QA Wolf user that launched the runner. A runner launched with a team API key belongs to the team's automation user and shows a person the page but not the screen, so the printed line promises the screen only when you signed in through the browser and this command is what launched the runner; attaching to a runner already running under that id offers the page without the promise, since someone else may have started it.
